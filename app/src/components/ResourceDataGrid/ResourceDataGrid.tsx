...
export const ResourceDataGrid = <T extends { id: string }>({ config }: ResourceDataGridProps<T>) => {
  ...
  return (
    <Box p={2} borderRadius={2} boxShadow="2px 2px 6px #ddd" display="flex" flexDirection="column" gap={2}>
      <Typography variant="h6">{config.label}</Typography>
      ...
      <Box display="flex" justifyContent="flex-start">
        <Button color="primary" startIcon={<AddIcon />} onClick={handleAddClick} variant="contained">
          Додати запис
        </Button>
      </Box>
      <Snackbar
