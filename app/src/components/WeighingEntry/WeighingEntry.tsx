...
export const WeighingsEntryForm = ({ weighingEntry = {}, id }) => {
  ...
  const handleSubmit = (e) => {
    ...
    switch (submitAction) {
      case 'add':
        ...
        if (!auto.id && String(auto.id).length === 0 && String(auto.id) !== '0')
          errorMessage.push('"auto.id" must be a string');
        if (!auto.driver && String(auto.driver).length === 0 && String(auto.driver) !== '0')
          errorMessage.push('"auto.driver" must be a string');
        if (!weighing.tare && String(weighing.tare).length === 0 && String(weighing.tare) !== '0')
          errorMessage.push('"weighing.tare" must be a Dnumber');
        if (!weighing.brutto && String(weighing.brutto).length === 0 && String(weighing.brutto) !== '0')
          errorMessage.push('"weighing.brutto" must be a string');
  ...
  return (
    <form
      className="flex justify-between gap-4 border border-gray-400 rounded-lg py-2 px-3 mt-2 mt- bg-[#eef376]"
    ...
    >
      {
        <>
            ...
            <div className="flex gap-4 justify-start">
              <div className="flex gap-4 flex-wrap min-w-[350px] w-fit">
                <span className="inline-block min-w-[95px]">
                  ({String(weighingEntry.date.day).padStart(2, '0')}/{String(weighingEntry.date.month).padStart(2, '0')}
                  /{String(weighingEntry.date.year)})
                </span>
              ...
              <label className="">
                {!!driversList && (
                  <select
                    name="driver"
                    className="w-fit"
                    defaultValue={weighingEntry.auto.driver || '0'}
                    onChange={handleModifyDriver}
                  >
                ...
                {!!autosList && (
                  <>
                    <select
                      name="auto"
                      className="w-fit"
                      defaultValue={weighingEntry.auto.id || '0'}
                      onChange={handleModifyAuto}
                    >
                      <option value="0" disabled>
                        - авто -
                      </option>
          ...
          <div className=" flex flex-col justify-around">
            ...
            <button
              type="submit"
              className="border border-black rounded-lg py-[2px] px-[30px] bg-[#b2e74e]"
            ...
            <button
              type="submit"
              className="border border-black rounded-lg py-[2px] px-[30px] bg-[#f38b77]"
